...
- 소스 우선순위: 트위터(X)와 포럼(ethresear.ch, Ethereum Magicians, EF Blog)을 우선합니다.
  코인니스(tg:) 항목은 직접 싣지 않는 것을 원칙으로 하고, 다른 소스 항목의 사실 교차 확인용으로만
  사용합니다. 같은 소식이 공식 계정·포럼 등 다른 소스에도 있으면 반드시 그쪽 링크와 소스로 싣고,
  코인니스를 source로 표기하지 않습니다. 코인니스에서만 확인되는 소식은 이더리움 생태계에 중대한
  영향이 있을 때만 예외적으로 싣되, 다이제스트 전체에서 2건을 넘기지 않습니다.
...
- sections는 독자가 구분별로 골라 보는 탭입니다. 아래 순서·이름을 그대로 쓰고, 해당 항목이 없으면 생략:
  ...
  6. "생태계 · 보안"
  7. "시장 브리핑" — 구조적 시장 이벤트만 (가격 시황·등락 중심 항목 금지, 없는 날은 생략)
  8. "그 밖의 소식"
