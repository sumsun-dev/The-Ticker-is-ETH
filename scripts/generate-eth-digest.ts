...
- source가 forkcast인 항목은 코어 개발자 콜(ACDE, ACDC, ACDT, 브레이크아웃)의 공식 기록입니다. "## 코어 개발자 콜" 섹션에 콜별로 핵심 결정(EIP 번호와 포크, 상태 변경을 원문 그대로), 일정, 액션 아이템, 영상 URL을 정리합니다. 요약하면서 결정을 빼먹지 않습니다.
...
- 인명은 통용되는 한글 표기가 확실할 때만 한글로 쓰고, 조금이라도 불확실하면 영문 그대로 씁니다.
  어색한 음차(예: 잘못된 철자 조합)를 만들어내지 않습니다.
- 대시(—) 구두점과 이모지는 쓰지 않습니다. 한국어에서 잘 쓰지 않는 표기이므로 쉼표나
...
- 가격·시세 내용은 최대한 자제합니다: 등락률·가격 전망·단순 시세 뉴스는 싣지 않습니다.
  시장 소식은 구조적 사건(ETF·기관·규제·대형 프로토콜 자금 흐름)만 다루고, 그때도 가격 수치
  나열이 아니라 생태계 함의 중심으로 서술합니다.
...
- sections는 독자가 구분별로 골라 보는 탭입니다. 아래 순서·이름을 그대로 쓰고, 해당 항목이 없으면 생략:
  1. "이번 호 인사이트" (필수)
  2. "코어 개발자 콜" — 브리핑에 코어 개발자 콜 기록이 있을 때만. 콜마다 항목 하나: title은 콜 이름과 날짜, summary는 핵심 결정(EIP 번호·포크·상태 변경 그대로)과 일정, why는 그 결정이 로드맵에 갖는 의미, url은 Forkcast 콜 페이지, source는 Forkcast
  3. "프로토콜 업데이트" — 스펙·하드포크·클라이언트·EF 공지 등 프로토콜 레벨 변화 (콜 결정과 중복되는 내용은 콜 섹션에만)
  4. "포럼 제안" — 새 EIP/ERC/리서치 제안 (Ethereum Magicians, ethresear.ch)
  5. "트위터 논쟁" — 설전·상반된 입장의 담론
  6. "주요 발언" — 핵심 인사의 주목할 단독 발언·글 (논쟁이 아닌 것)
  7. "생태계 · 보안"
  8. "시장 브리핑" — 구조적 시장 이벤트만 (가격 시황·등락 중심 항목 금지, 없는 날은 생략)
  9. "그 밖의 소식"
